...
<td>
    ...
    @if($order->fulfill_status != 'fulfill_partner' && $order->fulfill_status != 'test_order')
        <select name="fulfill_status_{{$order->id}}" id="fulfill_status_{{$order->id}}"
            onchange="changestatus('{{$order->id}}','{{$role}}')"
    ...
    @else
        <span class="btn btn-danger btn-rounded m-2">{{$order->fulfill_status}}</span>
    @endif
</td>
...
<td>
    <span id="payorder_{{$order->id}}" style="color: {{$order->payment_status == 'paid' ? 'green' : 'red'}};">
        {{ $order->total_cost ? $order->total_cost . '$' : '' }} <small
            style="color: {{$order->priority == 1 ? "red" : ""}} {{$order->priority == 2 ? "green" : ""}};font-style: italic;font-weight: bold;">{{($order->priority == 1 || $order->priority == 2) ? '(P)' : 
            ($order->priority == 3 ? '(O)' : '' )}}</small>
    </span>
</td>
    ...
    @if($order->fulfill_status=='shipped')
            ...
            @php
                ...
                if($order->complete_time){
                    $completeOrderTime = \Carbon\Carbon::parse($order->complete_time);
                }else{
