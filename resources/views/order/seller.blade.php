        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="form-group">
                    <select class="form-control" id="storeCreate" name="storeCreate" style="width: 190px;">
                ...
                <div style="display: flex; gap: 10px;" class="input-group">
                    ...
                    <button class="btn btn-primary" id="importCsv">Import CSV</button>
                </div>
                <!-- Maintenance -->
        ...
        <div class="card-body">
            ...
            <div class="pt-3">
                ...
                <div class="pb-2" id="sync_driver" style="display: flex; align-items: center; display: none">
                    <button type="submit" class="btn btn-primary m-1 btn-rounded" onclick="checkPayOrder()">Check Payorder</button>
                    <button type="submit" class="btn btn-success mr-1 btn-rounded"  id="pay_label_all"  onclick="payOrder()">Pay Order</button>
                </div>
                <table class="table table-striped table-borderless" id="data-table-default">
                    <thead>
                        <tr>
                            ...
                            <th style="min-width: 100px">Paid Cost</th>
...
<script>
    ...
    function checkboxorders() {
        syncToDriver.style.display = 'inline';
    ...
    function check_ff($order_id){
        // open new tab 
        window.open('/order/check-ff/'+$order_id, '_blank');
    }
    ...
    function checkPayOrder() {
        ...
        checkboxes.forEach(function (checkbox) {
            ...
            var styleColor = payorder.style.color;
    ...
    function changestatus(id, role) {
        ...
        if(status=='oversize'){
            $("#update_order_overize").prop('disabled', false);
    ...
    function update_order_overize(){
        ...
        var orderid = $('#id_chooseside').val();
        console.log(checkedIds);
        console.log(orderid);
        if(totalsize==0){
            alert("please select side oversize");
        }else{
            $("#update_order_overize").prop('disabled', true);
            $.ajax({
                ...
                error: function (response) {
                    alert("Order no change Oversize");
                }
            });
        }
    ...
    totalOrderTotay = {{Setting('new_order') + Setting('production')}};
    console.log("totalOrderTotay");
    console.log(totalOrderTotay);
    if(totalOrderTotay >= 5000){
        $('#ajaxCountOrderToday').html(`<div class="alert alert-danger alert-notification">
                            <i class="fa fa-check"></i>
                            The system reached capacity during the day, consider fulfilling more orders
                        </div>`);
    }
    </script>
@stop
