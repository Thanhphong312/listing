    ...
    @foreach ($json as $key => $file)
        @if(in_array($key, $listhumanid))
            <figure class="col-3 mt-2 position-relative">
                <a target="_blank">
                    <img src="https://global24watermark.site/teespring/generate/{{$type}}/{{$key}}/design.png"
                        onclick="chooseDesignHuman(this)" data-type="{{$type}}" data-key="{{$key}}" class="img-thumbnail"
