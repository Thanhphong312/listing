    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="m-1">
                <div class="row">
                    <div class="col-md-6">
                        <a type="button" class="btn btn-primary" href="{{route('templates.add')}}">  <i class="fas fa-plus"></i> Add Templete</a>
                        ...
                        @foreach($templetes as $templete)
                            <tr>
                                ...
                                <div class="d-flex flex-wrap align-items-start justify-content-start w-100">
                                    @php 
                                        $gen = json_decode($templete->data);

                                        if ($gen) {
                                            $json  =  $gen->product??"";
                                            if($json){
                                                $imagefirst = $json->image->src ?? './assets/img/image-default.png';
                                                $imagesizechar =  isset($json->imagesizechart) ?$json->imagesizechart: './assets/img/image-default.png';
                                                $pricefirst = is_array($json->variants)?isset($json->variants[0])?$json->variants[0]->price : 0:$json->variants->only_price;

                                                // Check if `options` array has enough elements before accessing each one
                                                $styles = isset($json->options[0]) ? $json->options[0]->values : [];
                                                $colors = isset($json->options[1]) ? $json->options[1]->values : [];
                                                $sizes = isset($json->options[2]) ? $json->options[2]->values : [];
