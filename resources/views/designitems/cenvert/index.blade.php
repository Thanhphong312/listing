...
<li class="breadcrumb-item active">
    <a href="{{ route('ideas.index')}}">@lang('Design Items')</a>
</li>
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-12 bg-secondary m-2 rounded" style="border: 2px solid;">
                    ...
                    <button id="generate" type="button" class="btn btn-danger btn-sm m-2" onclick="generate()"
                        disabled>gen</button>
                ...
                <div class="col-12 bg-light m-2 rounded row" style="border: 2px solid;">
                    <div class="col-7">
                        <input type="text" class="form-control m-2" placeholder="bank url" id="title" name="title"
                            value="{{$title}}">
                    </div>
                </div>
                <div class="col-12 bg-light m-2 rounded row" style="border: 2px solid;">
                    ...
                    <div class="col-5">
                        <input type="text" class="form-control m-2" placeholder="front url" value="{{$front_url}}">
                    </div>
                    <div class="col-5">
                        <input type="text" class="form-control m-2" placeholder="bank url" value="{{$back_url}}">
                    </div>
                </div>
                <div class="col-12 bg-light m-2 rounded" style="border: 2px solid;">
                    ...
                    @foreach ($colors as $color)
                        <span id="color_{{$color->id}}"
                            ...
                            class="btn btn-square-md m-1" data-color="{{$color->hex}}"
                            onclick="chooseColor('{{$color->id}}','{{$color->name}}','{{$color->hex}}')"></span>
                    @endforeach
                ...
                <div class="col-12 bg-light m-2 rounded row" style="border: 2px solid; min-height:200px;" id="mockup">
...
<script>
    ...
    let listcolorchoose = [];
    let linkfront = "{{$front_url}}";
    let linkback = "{{$back_url}}";
    let linksleeveleft = "{{$sleeve_left_url}}";
    let linksleeveright = "{{$sleeve_right_url}}";
    let mockupchoose = [];
    ...
    const numberside = [linkfront, linkback, linksleeveleft, linksleeveright].filter(link => link !== "").length;
    ...
    function chooseType(name) {
        ...
        $.ajax({
            url: '{{route("designItems.ajaxMockup")}}',
            method: 'get',
            data: {
                name: name,
    ...
    function chooseColor(id, name, hex) {
        console.log(hex)
        if ($("#color_" + id).hasClass("active-color")) {
            $("#color_" + id).removeClass("active-color");
            listcolorchoose = listcolorchoose.filter(item => item !== hex);
        } else {
            $("#color_" + id).addClass("active-color");
            listcolorchoose.push(hex);
        }
    }
    chooseTypeColor('all')
    ...
    function setup() {
        ...
        if (mockupUrlchoose == "") {
            ...
            $.ajax({
                url: '{{route("designItems.genPositions")}}',
                method: 'get',
                data: {
    ...
    function setuphuman() {
        ...
        if (mockupUrlchoosehuman == "") {
            ...
            $.ajax({
                url: '{{route("designItems.genPositionhumans")}}',
                method: 'get',
                data: {
    ...
    function generate() {
        ...
        listmockupchoose.forEach((mockup) => {
            listcolorchoose.forEach((color) => {
                ...
                if ($("#numberside").val() == 1) {
                    if (linkfront != "") {
                        mockupUrl = `https://global24watermark.site/generateposition/${mockup.dataType}/${color.slice(1)}/one/${mockup.dataKey}.jpg?url_1=${linkfront}?w=120&h=150&ver=${randomver}&position=${position}`;
                    }
                    if (linkback != "") {
                        mockupUrl = `https://global24watermark.site/generateposition/${mockup.dataType}/${color.slice(1)}/one/${mockup.dataKey}.jpg?url_1=${linkback}?w=120&h=150&ver=${randomver}&position=${position}`;
                    }
                } else {
                    mockupUrl = `https://global24watermark.site/generateposition/${mockup.dataType}/${color.slice(1)}/two/${mockup.dataKey}.jpg?url_1=${linkfront}?w=120&h=150&ver=${randomver}&url_2=${linkback}?w=120&h=150&ver=${randomver}&position=${position}`;
                }
    ...
    function next() {
        ...
        if (firstmockup == "") {
            ...
            $.ajax({
                url: '{{route("designItems.ajaxMockupHuman")}}',
                method: 'get',
                data: {
                    name: type,
                    numberside: numberside
    ...
    function post() {
        ...
        $.ajax({
            url: '{{ route("designItems.createProductDesign") }}',
            method: 'POST',
            headers: {
