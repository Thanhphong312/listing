    ...
    public function create(CountryRepository $countryRepository, RoleRepository $roleRepository)
    {
        ...
        if($user->role->name=="Seller"){
            $roles = Role::select('id','name')
                ->where('name','<>','User')
                ->where('id',5)
                ->orderByRaw("CASE WHEN name = 'Seller' THEN 0 ELSE 1 END")
                ->pluck('name', 'id')->toArray();
        }else{
            $roles = Role::select('id','name')->where('name','<>','User')->orderByRaw("CASE WHEN name = 'Seller' THEN 0 ELSE 1 END")->pluck('name', 'id')->toArray();
        }
