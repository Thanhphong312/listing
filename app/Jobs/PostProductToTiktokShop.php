        ...
        foreach ($variants as $variant) {
            ...
            if(isset($variant->option2)){
                ...
                if(isset($variant->options)){
                    $color = $this->findColor($variant->options);
                    if($color){
                        $imgurl = $this->findImageId($color);
                    }
                }
            }
            if ($imgurl) {
            ...
            if(count($this->variant_keys) > 0){
                $sales_attributes = [];
                foreach ($this->variant_keys as $key => $value) {
                    $sales_attributes[] = [
                        "id" => (string)$value->id,
                        "name" => (string)$value->name,
                        "value_name" => $variant->{$value->value_name} 
                    ];
                }
            }else{
